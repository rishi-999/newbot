...
namespace gazebo
{
  class PlannarMover : public ModelPlugin
  {
    ...
    public: void OnUpdate()
    {
        ...
        // Compute the y-axis position using a sine wave of time
        double y_pos = 0.25 * sin(2 * M_PI * 0.05 * current_time);

        // Compute the velocity needed to reach the desired position
        double y_vel = (y_pos - this->model->WorldPose().Pos().Y()) / 0.01; // assuming 100 Hz update rate

        // Set the velocity of the model in the y-axis direction
        this->model->SetLinearVel(ignition::math::Vector3d(0, y_vel, 0));
      }
